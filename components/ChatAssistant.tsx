import { VILLAS } from '@/constants/villaData';
...
// ─── Bot brain ────────────────────────────────────────────────────────────────
function getBotReply(input: string): { text: string; chips?: string[]; bookingVillaId?: string } {
  const msg = input.toLowerCase().trim();

  if (/^(hi|hello|hey|good\s*(morning|afternoon|evening)|kumusta|musta)/.test(msg)) {
    return { text: "Hi there! 👋 I'm Luna, your booking assistant for Casa Luna Villas. How can I help you today?", chips: ['View villas', 'Check availability', 'Pricing', 'Amenities'] };
  }
  if (/villa|property|properties|list|available|show/.test(msg)) {
    const list = VILLAS.map((v) => `• ${v.name} — ${v.location}`).join('\n');
    return { text: `We have ${VILLAS.length} beautiful villas:\n\n${list}\n\nWhich one interests you?`, chips: VILLAS.map((v) => v.name) };
  }
  // Book intents MUST come before info intents so "Book Pampanga" doesn't match /pampanga/ first
  if (/book silang|book.*silang|silang.*book/.test(msg)) {
    return { text: `Great choice! 🎉 Here's the booking form for Casa Luna Silang Cavite. Select your dates on the calendar:`, chips: [], bookingVillaId: '1' };
  }
  if (/book pampanga|book.*pampanga|pampanga.*book/.test(msg)) {
    return { text: `Awesome! 🎉 Here's the booking form for Casa Luna Pampanga. Select your dates on the calendar:`, chips: [], bookingVillaId: '2' };
  }
  if (/book|reserve|reservation/.test(msg)) {
    return { text: `Which villa would you like to book?`, chips: ['Book Silang', 'Book Pampanga'] };
  }
  if (/silang|cavite/.test(msg)) {
    const v = VILLAS.find((x) => x.id === '1')!;
    return { text: `🏡 ${v.name}\n📍 ${v.location}\n👥 Up to ${v.guests} guests · 🛏 ${v.bedrooms} bedrooms\n💰 ₱${v.price.toLocaleString()}/night\n\n${v.description}\n\n✨ ${v.amenities.join(', ')}`, chips: ['Book Silang', 'Check pricing', 'View Pampanga villa'] };
  }
  if (/pampanga/.test(msg)) {
    const v = VILLAS.find((x) => x.id === '2')!;
    return { text: `🏡 ${v.name}\n📍 ${v.location}\n👥 Up to ${v.guests} guests · 🛏 ${v.bedrooms} bedrooms\n💰 ₱${v.price.toLocaleString()}/night\n\n${v.description}\n\n✨ ${v.amenities.join(', ')}`, chips: ['Book Pampanga', 'Check pricing', 'View Silang villa'] };
  }
  if (/price|pricing|rate|cost|how much|magkano/.test(msg)) {
    const lines = VILLAS.map((v) => `• ${v.name}: ₱${v.price.toLocaleString()}/night`).join('\n');
    return { text: `Current rates:\n\n${lines}\n\nWould you like to book?`, chips: ['Book Silang', 'Book Pampanga', 'Ask about discounts'] };
  }
  if (/amenities|amenity|facilities|pool|wifi|bbq|karaoke|parking/.test(msg)) {
    return { text: `Both villas include:\n\n🏊 Private Pool · 📶 WiFi · ❄️ Air Conditioning · 🍖 BBQ · 🚗 Parking · 🌿 Garden · 🍽 Outdoor Dining · 🎤 Karaoke`, chips: ['Book now', 'Pricing', 'How many guests?'] };
  }
  if (/guest|capacity|how many|people|pax/.test(msg)) {
    return { text: `Both villas accommodate up to 20 guests with 5 bedrooms. Perfect for reunions and events! 🎉`, chips: ['Book Silang', 'Book Pampanga', 'Pricing'] };
  }
  if (/discount|promo|sale|special|offer/.test(msg)) {
    return { text: `We occasionally offer special rates for extended stays. Check back soon or contact us directly! 😊`, chips: ['Pricing', 'Book now'] };
  }
  if (/contact|reach|call|facebook|fb/.test(msg)) {
    return { text: `Reach us through our Facebook page for custom arrangements and group bookings! 📱`, chips: ['Pricing', 'Book now'] };
  }
  if (/location|where|address|direction/.test(msg)) {
    return { text: `📍 Silang Cavite — ~1–1.5 hrs from Metro Manila via SLEX\n📍 Pampanga — ~1.5–2 hrs via NLEX\n\nExact address shared upon booking confirmation.`, chips: ['Book Silang', 'Book Pampanga'] };
  }
  if (/thank|thanks|salamat/.test(msg)) {
    return { text: `You're welcome! 😊 Feel free to ask anything else about Casa Luna!`, chips: ['View villas', 'Book now', 'Pricing'] };
  }
  return { text: `I'm happy to help with anything about Casa Luna Villas!`, chips: ['View villas', 'Pricing', 'Amenities', 'How to book'] };
}
...
const INITIAL_MESSAGE: Message = {
  id: '0', role: 'bot',
  text: "Hi! 👋 I'm Luna, your Casa Luna booking assistant. How can I help you today?",
  chips: ['View villas', 'Pricing', 'Amenities', 'How to book'],
};
...
export default function ChatAssistant() {
  ...
  const addMessage = (role: Role, text: string, chips?: string[], bookingVillaId?: string) => {
    setMessages((prev) => [...prev, { id: Date.now().toString(), role, text, chips, bookingVillaId }]);
    scrollToEnd();
  };
  ...
  const handleSend = (text: string) => {
    ...
    setTimeout(() => {
      ...
      addMessage('bot', reply.text, reply.chips, reply.bookingVillaId);
    }, 400);
  };
